...
class Server {
	...
	/**
	 * Retrieves a scrobbles by uniqueid
	 *
	 ...
	 */
	static function getSingleScrobble($userid = 3, $scrobble = "1"){
		global $adodb;
		...
		if ($userid) {
			...
			$query = 'SELECT s.* FROM Scrobbles s WHERE s.userid=' . $userid . ' AND s.time=' . $adodb->qstr($scrobble);
		}

		try {
			$res = $adodb->CacheGetAll(60, $query);
		} catch (Exception $e) {
